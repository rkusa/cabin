    ...
    ($method_name:ident, $marker_name:ident, $doc:literal) => {
        pub mod $method_name {
            ...
            pub mod marker {
                ...
                impl<'v, V: crate::View + 'v> crate::element::IntoChild<'v, $marker_name> for V {
...
vanilla_element!(
    ...
    "The `article` element represents a complete, or self-contained, composition in a document, \
     ...
     reusable, e.g. in syndication. This could be a forum post, a magazine or newspaper article, \
...
vanilla_element!(
    ...
    "The `b` element represents a span of text to which attention is being drawn for utilitarian \
     purposes without conveying any extra importance and with no implication of an alternate \
...
vanilla_element!(
    ...
    "The `footer` element represents a footer for its nearest ancestor sectioning content \
     element, or for the body element if there is no such ancestor. A footer typically contains \
...
vanilla_element!(
    ...
    "The `i` element represents a span of text in an alternate voice or mood, or otherwise offset \
     from the normal prose in a manner indicating a different quality of text, such as a \
...
vanilla_element!(
    ...
    "The `li` element represents a list item. If its parent element is an [fn@h::ol], [fn@h::ul], \
     or [h::menu] element, then the element is an item of the parent element's list, as defined \
...
vanilla_element!(
    ...
    "The `mark` element represents a run of text in one document marked or highlighted for \
     reference purposes, due to its relevance in another context. When used in a quotation or \
     other block of text referred to from the prose, it indicates a highlight that was not \
     originally present but which has been added to bring the reader's attention to a part of the \
     text that might not have been considered important by the original author when the block was \
     originally written, but which is now under previously unexpected scrutiny. When used in the \
...
vanilla_element!(
    ...
    "The `noscript` element represents nothing if scripting is enabled, and represents its \
     children if scripting is disabled. It is used to present different markup to user agents \
...
vanilla_element!(
    ...
    "The `picture` element is a container which provides multiple sources to its contained \
     [fn@h::img] element to allow authors to declaratively control or give hints to the user \
...
vanilla_element!(
    ...
    "The `ruby` element allows one or more spans of phrasing content to be marked with ruby \
     annotations. Ruby annotations are short runs of text presented alongside base text, \
...
vanilla_element!(
    ...
    "The `var` element represents a variable. This could be an actual variable in a mathematical \
     expression or programming context, an identifier representing a constant, a symbol \
     identifying a physical quantity, a function parameter, or just be a term used as a \
